...
public class OrderPanel extends JPanel {
	...
	private void createUI() {
		...
		JCheckBox bankTransferCheckBox = new JCheckBox("Thanh toán chuyển khoản");
	...
	public void addOrderItem(HangHoa hh, int quantity) {
		...
		Object[] row = { hh.getTenHH(), quantity, price, // Lưu Double
				total, // Lưu Double
				"Xóa" };
			...
			try (Connection con = ConnectDB.getConnection(); PreparedStatement stmt = con.prepareStatement(sql)) {
				...
				if (rs.next()) {
					...
					JOptionPane.showMessageDialog(this,
							"Áp dụng mã giảm giá thành công! Giảm " + (int) discountPercentage + "%");
				} else {
